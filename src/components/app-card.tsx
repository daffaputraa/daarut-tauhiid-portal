...
export default function CardApp({ logo, title, description, link, button, width, height }: CardAppProps) {
    return (
      <Link href={link} className="min-h-full">
        <div className="relative flex flex-col min-h-full justify-between rounded-xl border border-stone-200 bg-white/50 p-4 shadow-sm transition hover:shadow-md hover:bg-stone-100 hover:scale-105 cursor-pointer">
          ...
          <Link
            // target="_blank"
            href={link}
            className={`mt-2 text-center font-bold rounded-lg w-full ${button} px-4 py-2 text-white shadow hover:bg-stone-700 transition`}
          >
            Masuk
          </Link>
        </div>
